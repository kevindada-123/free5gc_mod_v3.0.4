...
func (policy *UeSmPolicyData) CheckRelatedAfEvent(event models.AfEvent) (found bool) {
	for appSessionId := range policy.AppSessions {
		if val, ok := SMAF_Self().AppSessionPool.Load(appSessionId); ok {
...
func (ue *PCFUeContext) AllocUeAppSessionId(context *SMAFContext) string {
	var allocID int64
	var err error
	if allocID, err = ue.AppSessionIDGenerator.Allocate(); err != nil {
		logger.SMAFContextLog.Warnf("Allocate AppSessionId error: %+v", err)
		return ""
	}
