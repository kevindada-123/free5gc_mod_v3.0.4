...
func GetDefaultTime() models.TimeWindow {
	...
	if err != nil {
		logger.PCFUtilLog.Errorf("ParseDuration error: %+v", err)
	}
...
func MarshToJsonString(v interface{}) (result []string) {
	...
	if types.Kind() == reflect.Slice {
		for i := 0; i < val.Len(); i++ {
			...
			if err != nil {
				logger.PCFUtilLog.Errorf("Marshal error: %+v", err)
			}
...
func GetNegotiateSuppFeat(suppFeat string, serviceSuppFeat []byte) string {
	...
	if err != nil {
		logger.PCFUtilLog.Errorf("DecodeString error: %+v", err)
	}
