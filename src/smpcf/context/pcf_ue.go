...
func (ue *PCFUeContext) NewUeAMPolicyData(assolId string, req models.PolicyAssociationRequest) *UeAMPolicyData {
	ue.Gpsi = req.Gpsi
	ue.Pei = req.Pei
...
func (ue *PCFUeContext) NewUeSmPolicyData(
...
func (policy *UeSmPolicyData) CheckRelatedAfEvent(event models.AfEvent) (found bool) {
	for appSessionId := range policy.AppSessions {
		if val, ok := SMPCF_Self().AppSessionPool.Load(appSessionId); ok {
...
func (ue *PCFUeContext) FindAMPolicy(anType models.AccessType, plmnId *models.NetworkId) *UeAMPolicyData {
...
func (ue *PCFUeContext) AllocUeAppSessionId(context *SMPCFContext) string {
	var allocID int64
	var err error
	if allocID, err = ue.AppSessionIDGenerator.Allocate(); err != nil {
		logger.SMPCFContextLog.Warnf("Allocate AppSessionId error: %+v", err)
		return ""
	}
...
func (ue *PCFUeContext) SMPolicyFindByIpv4(v4 string) *UeSmPolicyData {
...
func (ue *PCFUeContext) SMPolicyFindByIpv6(v6 string) *UeSmPolicyData {
	for _, smPolicy := range ue.SmPolicyData {
...
// returns SM Policy by IPv4
func (ue *PCFUeContext) SMPolicyFindByIdentifiersIpv4(
...
// returns SM Policy by IPv6
func (ue *PCFUeContext) SMPolicyFindByIdentifiersIpv6(v6 string, sNssai *models.Snssai, dnn string) *UeSmPolicyData {
	for _, smPolicy := range ue.SmPolicyData {
		policyContext := smPolicy.PolicyContext
