...
func (node *DataPathNode) ActivateUpLinkTunnel(smContext *SMContext) error {
	var err error
	logger.SMPCFContextLog.Traceln("In ActivateUpLinkTunnel")
	...
	if err != nil {
		logger.SMPCFContextLog.Warnln("In ActivateUpLinkTunnel UPF IP: ", node.UPF.NodeID.ResolveNodeIdToIp().String())
		logger.SMPCFContextLog.Warnln("Allocata PDR Error: ", err)
	}

	smContext.PutPDRtoPFCPSession(destUPF.NodeID, node.UpLinkTunnel.PDR)

	if teid, err := destUPF.GenerateTEID(); err != nil {
		logger.SMPCFContextLog.Errorf("Generate uplink TEID fail: %s", err)
		return err
	} else {
...
func (node *DataPathNode) ActivateDownLinkTunnel(smContext *SMContext) error {
	...
	if newPDR, err := destUPF.AddPDR(); err != nil {
		logger.SMPCFContextLog.Warnln("In ActivateDownLinkTunnel UPF IP: ", node.UPF.NodeID.ResolveNodeIdToIp().String())
		logger.SMPCFContextLog.Warnln("Allocata PDR Error: ", err)
	...
	if teid, err := destUPF.GenerateTEID(); err != nil {
		logger.SMPCFContextLog.Errorf("Generate downlink TEID fail: %s", err)
		return err
	} else {
...
func (node *DataPathNode) DeactivateUpLinkTunnel(smContext *SMContext) {
	...
	if err != nil {
		logger.SMPCFContextLog.Warnln("Deactivaed UpLinkTunnel", err)
	}
...
func (node *DataPathNode) DeactivateDownLinkTunnel(smContext *SMContext) {
	...
	err := node.UPF.RemovePDR(pdr)
	if err != nil {
		logger.SMPCFContextLog.Warnln("Deactivaed DownLinkTunnel", err)
	}
...
func (node *DataPathNode) GetUPFID() (id string, err error) {
	...
	if id, exist = smpcfContext.UserPlaneInformation.UPFsIPtoID[node_ip]; !exist {
...
func (dataPath *DataPath) ActivateTunnelAndPDR(smContext *SMContext) {
	...
	//Activate PDR
	for curDataPathNode := firstDPNode; curDataPathNode != nil; curDataPathNode = curDataPathNode.Next() {
		logger.SMPCFContextLog.Traceln("Calculate ", curDataPathNode.UPF.PFCPAddr().String())
