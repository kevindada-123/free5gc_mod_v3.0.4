...
func AllocateUPFID() {
	UPFsID := smpcfContext.UserPlaneInformation.UPFsID
	UPFsIPtoID := smpcfContext.UserPlaneInformation.UPFsIPtoID

	for upfName, upfNode := range smpcfContext.UserPlaneInformation.UPFs {
...
func GenerateDataPath(upPath UPPath, smContext *SMContext) *DataPath {
	if len(upPath) < 1 {
		logger.SMPCFContextLog.Errorf("Invalid data path")
		return nil
	}
	var lowerBound = 0
...
func (upi *UserPlaneInformation) GenerateDefaultPath(dnn string) bool {
	...
	if source == nil {
		logger.SMPCFContextLog.Errorf("There is no AN Node in config file!")
		return false
	}
	...
	if destination == nil {
		logger.SMPCFContextLog.Errorf("Can't find UPF with DNN [%s]\n", dnn)
		return false
	}
