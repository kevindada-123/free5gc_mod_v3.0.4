...
func handleSessionRule(smContext *smpcf_context.SMContext, id string, sessionRuleModel *models.SessionRule) {
	if sessionRuleModel == nil {
		logger.PduSessLog.Debugf("Delete SessionRule[%s]", id)
		delete(smContext.SessionRules, id)
	} else {
		sessRule := smpcf_context.NewSessionRuleFromModel(sessionRuleModel)
...
func ApplySmPolicyFromDecision(smContext *smpcf_context.SMContext, decision *models.SmPolicyDecision) error {
	...
	if selectedSessionRule == nil { //No active session rule
		...
		for id := range smContext.SessionRules {
			...
			smpcf_context.SetSessionRuleActivateState(smContext.SessionRules[id], true)
			break
		}
	} else {
		...
		if _, exist := smContext.SessionRules[selectedSessionRuleID]; !exist {
			...
			smpcf_context.SetSessionRuleActivateState(smContext.SessionRules[selectedSessionRuleID], true)
		}
	}
