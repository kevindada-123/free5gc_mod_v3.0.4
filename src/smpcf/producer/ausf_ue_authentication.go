...
func UeAuthPostRequestProcedure(updateAuthenticationInfo models.AuthenticationInfo) (*models.UeAuthenticationCtx,
	...
	servingNetworkAuthorized := smpcf_context.IsServingNetworkAuthorized(snName)
	...
	self := smpcf_context.SMPCF_Self()
	authInfoReq.AusfInstanceId = self.SMPCF_SelfID()
	...
	if updateAuthenticationInfo.ResynchronizationInfo != nil {
		...
		ausfCurrentSupi := smpcf_context.GetSupiFromSuciSupiMap(supiOrSuci)
		logger.UeAuthPostLog.Warningln(ausfCurrentSupi)
		ausfCurrentContext := smpcf_context.GetAusfUeContext(ausfCurrentSupi)
	...
	if authInfoResult.AuthType == models.AuthType__5_G_AKA {
		...
		atMacNum := fmt.Sprintf("%02x", smpcf_context.AT_MAC_ATTRIBUTE)
...
func Auth5gAkaComfirmRequestProcedure(updateConfirmationData models.ConfirmationData,
	...
	if !smpcf_context.CheckIfSuciSupiPairExists(ConfirmationDataResponseID) {
	...
	currentSupi := smpcf_context.GetSupiFromSuciSupiMap(ConfirmationDataResponseID)
	if !smpcf_context.CheckIfAusfUeContextExists(currentSupi) {
	...
	ausfCurrentContext := smpcf_context.GetAusfUeContext(currentSupi)
...
func EapAuthComfirmRequestProcedure(updateEapSession models.EapSession, eapSessionID string) (*models.EapSession,
	*models.ProblemDetails) {
	...
	if !smpcf_context.CheckIfSuciSupiPairExists(eapSessionID) {
	...
	currentSupi := smpcf_context.GetSupiFromSuciSupiMap(eapSessionID)
	if !smpcf_context.CheckIfAusfUeContextExists(currentSupi) {
