...
import (
	...
	pfcp_message "free5gc/src/smpcf/pfcp/message"
	"net/http"
...
func HandlePDUSessionSMContextCreate(request models.PostSmContextsRequest) *http_wrapper.Response {
	...
	smContext := smpcf_context.NewSMContext(createData.Supi, createData.PduSessionId)
	smContext.SMContextState = smpcf_context.ActivePending
	logger.SMPCFContextLog.Traceln("SMContextState Change State: ", smContext.SMContextState.String())
	...
	SubscriberDataManagementClient := smpcf_context.SMPCF_Self().SubscriberDataManagementClient
	...
	smPolicyData.NotificationUri = fmt.Sprintf("%s://%s:%d/nsmf-callback/sm-policies/%s",
		smpcf_context.SMPCF_Self().UriScheme,
		smpcf_context.SMPCF_Self().RegisterIPv4,
		smpcf_context.SMPCF_Self().SBIPort,
		smContext.Ref,
	)
	...
	*/
	...
	smContext.Tunnel = smpcf_context.NewUPTunnel()
	var defaultPath *smpcf_context.DataPath

	if smpcf_context.SMPCF_Self().ULCLSupport && smpcf_context.CheckUEHasPreConfig(createData.Supi) {
		logger.PduSessLog.Infof("SUPI[%s] has pre-config route", createData.Supi)
		uePreConfigPaths := smpcf_context.GetUEPreConfigPaths(createData.Supi)
		...
		smContext.BPManager = smpcf_context.NewBPManager(createData.Supi)
	} else {
		//UE has no pre-config path.
		//Use default route
		logger.PduSessLog.Infof("SUPI[%s] has no pre-config route", createData.Supi)
		//fmt.Printf("createData : %#v \n", createData)
		defaultUPPath := smpcf_context.GetUserPlaneInformation().GetDefaultUserPlanePathByDNN(createData.Dnn)
		smContext.AllocateLocalSEIDForUPPath(defaultUPPath)
		defaultPath = smpcf_context.GenerateDataPath(defaultUPPath, smContext)
		if defaultPath != nil {
...
func HandlePDUSessionSMContextUpdate(smContextRef string, body models.UpdateSmContextRequest) *http_wrapper.Response {
	...
	smContext := smpcf_context.GetSMContext(smContextRef)
	...
	if body.BinaryDataN1SmMessage != nil {
		...
		logger.PduSessLog.Traceln("[SMPCF] UpdateSmContextRequest N1SmMessage: ", m)
		if err != nil {
		...
		case nas.MsgTypePDUSessionReleaseRequest:
			if smContext.SMContextState != smpcf_context.Active {
			...
			if buf, err := smpcf_context.BuildGSMPDUSessionReleaseCommand(smContext); err != nil {
			...
			if buf, err := smpcf_context.BuildPDUSessionResourceReleaseCommandTransfer(smContext); err != nil {
			...
			smContext.PendingUPF = make(smpcf_context.PendingUPF)
			...
			sendPFCPDelete = true
			smContext.SMContextState = smpcf_context.PFCPModification
			logger.SMPCFContextLog.Traceln("SMContextState Change State: ", smContext.SMContextState.String())
		case nas.MsgTypePDUSessionReleaseComplete:
			if smContext.SMContextState != smpcf_context.InActivePending {
				//Wait till the state becomes Active again
				//TODO: implement sleep wait in concurrent architecture
			...
			logger.PduSessLog.Infoln("[SMPCF] Send Update SmContext Response")
			smContext.SMContextState = smpcf_context.InActive
			logger.SMPCFContextLog.Traceln("SMContextState Change State: ", smContext.SMContextState.String())
			response.JsonData.UpCnxState = models.UpCnxState_DEACTIVATED
			smpcf_context.RemoveSMContext(smContext.Ref)
		...
		logger.PduSessLog.Traceln("[SMPCF] Binary Data N1 SmMessage is nil!")
	}

	tunnel := smContext.Tunnel
	pdrList := []*smpcf_context.PDR{}
	farList := []*smpcf_context.FAR{}
	barList := []*smpcf_context.BAR{}
	...
	case models.UpCnxState_ACTIVATING:
		...
		smContext.SMContextState = smpcf_context.ModificationPending
		logger.SMPCFContextLog.Traceln("SMContextState Change State: ", smContext.SMContextState.String())
		...
		n2Buf, err := smpcf_context.BuildPDUSessionResourceSetupRequestTransfer(smContext)
		if err != nil {
	...
	case models.UpCnxState_DEACTIVATED:
		...
		for _, dataPath := range smContext.Tunnel.DataPathPool {
			...
			if DLPDR == nil {
				...
				DLPDR.FAR.State = smpcf_context.RULE_UPDATE
	...
	case models.N2SmInfoType_PDU_RES_SETUP_RSP:
		...
		for _, dataPath := range tunnel.DataPathPool {
			...
			if dataPath.Activated {
				...
				DLPDR.FAR.ForwardingParameters = &smpcf_context.ForwardingParameters{
				...
				DLPDR.State = smpcf_context.RULE_UPDATE
				DLPDR.FAR.State = smpcf_context.RULE_UPDATE
	...
	case models.N2SmInfoType_PDU_RES_REL_RSP:
		logger.PduSessLog.Infoln("[SMPCF] N2 PDUSession Release Complete ")
		if smContext.PDUSessionRelease_DUE_TO_DUP_PDU_ID {
			...
			logger.PduSessLog.Infoln("[SMPCF] Send Update SmContext Response")
			smContext.SMContextState = smpcf_context.InActivePending
			logger.SMPCFContextLog.Traceln("SMContextState Change State: ", smContext.SMContextState.String())
	...
	case models.N2SmInfoType_PATH_SWITCH_REQ:
		...
		if err := smpcf_context.HandlePathSwitchRequestTransfer(body.BinaryDataN2SmInformation, smContext); err != nil {
			logger.PduSessLog.Errorf("Handle PathSwitchRequestTransfer: %+v", err)
		}

		if n2Buf, err := smpcf_context.BuildPathSwitchRequestAcknowledgeTransfer(smContext); err != nil {
	...
	case models.N2SmInfoType_PATH_SWITCH_SETUP_FAIL:
		...
		logger.SMPCFContextLog.Traceln("SMContextState Change State: ", smContext.SMContextState.String())
		if err :=
			smpcf_context.HandlePathSwitchRequestSetupFailedTransfer(body.BinaryDataN2SmInformation, smContext); err != nil {
	...
	case models.HoState_PREPARING:
		...
		if err := smpcf_context.HandleHandoverRequiredTransfer(body.BinaryDataN2SmInformation, smContext); err != nil {
			logger.PduSessLog.Errorf("Handle HandoverRequiredTransfer failed: %+v", err)
		}
		response.JsonData.N2SmInfoType = models.N2SmInfoType_PDU_RES_SETUP_REQ

		if n2Buf, err := smpcf_context.BuildPDUSessionResourceSetupRequestTransfer(smContext); err != nil {
			logger.PduSessLog.Errorf("Build PDUSession Resource Setup Request Transfer Error(%s)", err.Error())
		} else {
			response.BinaryDataN2SmInformation = n2Buf
		}
	...
	case models.HoState_PREPARED:
		...
		if err :=
			smpcf_context.HandleHandoverRequestAcknowledgeTransfer(body.BinaryDataN2SmInformation, smContext); err != nil {
			logger.PduSessLog.Errorf("Handle HandoverRequestAcknowledgeTransfer failed: %+v", err)
		}

		if n2Buf, err := smpcf_context.BuildHandoverCommandTransfer(smContext); err != nil {
			logger.PduSessLog.Errorf("Build PDUSession Resource Setup Request Transfer Error(%s)", err.Error())
		} else {
			response.BinaryDataN2SmInformation = n2Buf
		}
	...
	case models.Cause_REL_DUE_TO_DUPLICATE_SESSION_ID:
		...
		buf, err := smpcf_context.BuildPDUSessionResourceReleaseCommandTransfer(smContext)
		response.BinaryDataN2SmInformation = buf
		if err != nil {
			logger.PduSessLog.Error(err)
		}
		...
		smContext.SMContextState = smpcf_context.PFCPModification
		logger.SMPCFContextLog.Traceln("SMContextState Change State: ", smContext.SMContextState.String())
		logger.SMPCFContextLog.Infoln("[SMPCF] Cause_REL_DUE_TO_DUPLICATE_SESSION_ID")
	}

	var httpResponse *http_wrapper.Response
	//Check FSM and take corresponding action
	switch smContext.SMContextState {
	case smpcf_context.PFCPModification:
		logger.SMPCFContextLog.Traceln("In case PFCPModification")
		...
		case smpcf_context.SessionUpdateSuccess:
			logger.SMPCFContextLog.Traceln("In case SessionUpdateSuccess")
			smContext.SMContextState = smpcf_context.Active
			logger.SMPCFContextLog.Traceln("SMContextState Change State: ", smContext.SMContextState.String())
			httpResponse = &http_wrapper.Response{
				Status: http.StatusOK,
				Body:   response,
			}
		case smpcf_context.SessionUpdateFailed:
			logger.SMPCFContextLog.Traceln("In case SessionUpdateFailed")
			smContext.SMContextState = smpcf_context.Active
			logger.SMPCFContextLog.Traceln("SMContextState Change State: ", smContext.SMContextState.String())
		...
		case smpcf_context.SessionReleaseSuccess:
			logger.SMPCFContextLog.Traceln("In case SessionReleaseSuccess")
			smContext.SMContextState = smpcf_context.InActivePending
			logger.SMPCFContextLog.Traceln("SMContextState Change State: ", smContext.SMContextState.String())
			httpResponse = &http_wrapper.Response{
				Status: http.StatusOK,
		...
		case smpcf_context.SessionReleaseFailed:
			// Update SmContext Request(N1 PDU Session Release Request)
			// Send PDU Session Release Reject
			logger.SMPCFContextLog.Traceln("In case SessionReleaseFailed")
			...
			if buf, err := smpcf_context.BuildGSMPDUSessionReleaseReject(smContext); err != nil {
	...
	case smpcf_context.ModificationPending:
		logger.SMPCFContextLog.Traceln("In case ModificationPending")
		smContext.SMContextState = smpcf_context.Active
		logger.SMPCFContextLog.Traceln("SMContextState Change State: ", smContext.SMContextState.String())
		httpResponse = &http_wrapper.Response{
			Status: http.StatusOK,
	...
	case smpcf_context.InActive, smpcf_context.InActivePending:
		logger.SMPCFContextLog.Traceln("In case InActive, InActivePending")
		httpResponse = &http_wrapper.Response{
			Status: http.StatusOK,
			Body:   response,
		}
...
func HandlePDUSessionSMContextRelease(smContextRef string, body models.ReleaseSmContextRequest) *http_wrapper.Response {
	...
	case smpcf_context.SessionReleaseFailed:
		...
		httpResponse.Body = errResponse
	default:
		logger.SMPCFContextLog.Warnf("The state shouldn't be [%s]\n", PFCPResponseStatus)

		logger.SMPCFContextLog.Traceln("In case Unkown")
		problemDetail := models.ProblemDetails{
			Status: http.StatusInternalServerError,
			Cause:  "SYSTEM_FAILULE",
		}
...
func SendPFCPRule(smContext *smpcf_context.SMContext, dataPath *smpcf_context.DataPath) {

	logger.PduSessLog.Infoln("Send PFCP Rule")
	logger.PduSessLog.Infoln("DataPath: ", dataPath)
	for curDataPathNode := dataPath.FirstDPNode; curDataPathNode != nil; curDataPathNode = curDataPathNode.Next() {
		pdrList := make([]*smpcf_context.PDR, 0, 2)
		farList := make([]*smpcf_context.FAR, 0, 2)
