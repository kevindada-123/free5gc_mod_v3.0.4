...
func AddPDUSessionAnchorAndULCL(smContext *context.SMContext, nodeID pfcpType.NodeID) {
	...
	case context.EstablishingNewPSA:
		...
		if exist {
			...
			logger.SMPCFContextLog.Warnln("In AddPDUSessionAnchorAndULCL case EstablishingNewPSA")
			logger.SMPCFContextLog.Warnln("UPF IP ", trggierUPFIP, " doesn't exist in pending UPF!")
			return
		}
	...
	case context.EstablishingULCL:
		...
		if exist {
			...
			logger.SMPCFContextLog.Warnln("In AddPDUSessionAnchorAndULCL case EstablishingULCL")
			logger.SMPCFContextLog.Warnln("UPF IP ", trggierUPFIP, " doesn't exist in pending UPF!")
			return
		}
	...
	case context.UpdatingRANAndIUPFUpLink:
		...
		if exist {
			...
			logger.SMPCFContextLog.Warnln("In AddPDUSessionAnchorAndULCL case UpdatingRANAndIUPFUpLink")
			logger.SMPCFContextLog.Warnln("UPF IP ", trggierUPFIP, " doesn't exist in pending UPF!")
			return
		}
		...
		if pendingUPF.IsEmpty() {
			...
			logger.SMPCFContextLog.Infoln("[SMPCF] Add PSA success")
		}
	}
...
func EstablishULCL(smContext *context.SMContext) {
	...
	logger.PfcpLog.Info("[SMPCF] Establish ULCL msg has been send")
	...
	for curDataPathNode := activatingPath.FirstDPNode; curDataPathNode != nil; curDataPathNode = curDataPathNode.Next() {
		...
		if lastNode != nil {
			if reflect.DeepEqual(lastNode.UPF.NodeID, ulcl.NodeID) {
				...
				logger.PfcpLog.Info("[SMPCF] Update PSA2 downlink msg has been send")
				break
			}
		}
	}
