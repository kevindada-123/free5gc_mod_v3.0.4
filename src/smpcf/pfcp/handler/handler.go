...
func HandlePfcpAssociationSetupRequest(msg *pfcpUdp.Message) {
	...
	upf := smpcf_context.RetrieveUPFNodeByNodeID(*nodeID)
	if upf == nil {
...
func HandlePfcpAssociationSetupResponse(msg *pfcpUdp.Message) {
	...
	if req.Cause.CauseValue == pfcpType.CauseRequestAccepted {
		...
		upf := smpcf_context.RetrieveUPFNodeByNodeID(*req.NodeID)

		upf.UPFStatus = smpcf_context.AssociatedSetUpSuccess
...
func HandlePfcpAssociationReleaseRequest(msg *pfcpUdp.Message) {
	...
	upf := smpcf_context.RetrieveUPFNodeByNodeID(*pfcpMsg.NodeID)

	if upf != nil {
		smpcf_context.RemoveUPFNodeByNodeId(*pfcpMsg.NodeID)
...
func HandlePfcpSessionEstablishmentResponse(msg *pfcpUdp.Message) {
	...
	smContext := smpcf_context.GetSMContextBySEID(SEID)
	...
	if rsp.Cause.CauseValue == pfcpType.CauseRequestAccepted &&
		...
		if smNasBuf, err := smpcf_context.BuildGSMPDUSessionEstablishmentAccept(smContext); err != nil {
			logger.PduSessLog.Errorf("Build GSM PDUSessionEstablishmentAccept failed: %s", err)
		} else {
			n1n2Request.BinaryDataN1Message = smNasBuf
		}
		if n2Pdu, err := smpcf_context.BuildPDUSessionResourceSetupRequestTransfer(smContext); err != nil {
	...
	if smpcf_context.SMPCF_Self().ULCLSupport && smContext.BPManager != nil {
		if smContext.BPManager.BPStatus == smpcf_context.AddingPSA {
			logger.PfcpLog.Infoln("Keep Adding PSAndULCL")
			producer.AddPDUSessionAnchorAndULCL(smContext, *rsp.NodeID)
			smContext.BPManager.BPStatus = smpcf_context.AddingPSA
		}
	}
}
...
func HandlePfcpSessionModificationResponse(msg *pfcpUdp.Message) {
	...
	if pfcpRsp.Cause.CauseValue == pfcpType.CauseRequestAccepted {
		logger.PduSessLog.Infoln("[SMPCF] PFCP Modification Resonse Accept")
		if smContext.SMContextState == smpcf_context.PFCPModification {
			...
			if smContext.PendingUPF.IsEmpty() {
				smContext.SBIPFCPCommunicationChan <- smpcf_context.SessionUpdateSuccess
			}

			if smpcf_context.SMPCF_Self().ULCLSupport && smContext.BPManager != nil {
				if smContext.BPManager.BPStatus == smpcf_context.UnInitialized {
		...
		if smContext.SMContextState == smpcf_context.PFCPModification {
			smContext.SBIPFCPCommunicationChan <- smpcf_context.SessionUpdateFailed
		}
	}

	logger.SMPCFContextLog.Traceln("PFCP Session Context")
	for _, ctx := range smContext.PFCPContext {

		logger.SMPCFContextLog.Traceln(ctx.ToString())
	}
...
func HandlePfcpSessionDeletionResponse(msg *pfcpUdp.Message) {
	...
	if pfcpRsp.Cause.CauseValue == pfcpType.CauseRequestAccepted {
		if smContext.SMContextState == smpcf_context.PFCPModification {
			...
			if smContext.PendingUPF.IsEmpty() {
				smContext.SBIPFCPCommunicationChan <- smpcf_context.SessionReleaseSuccess
			}
		}
		logger.PfcpLog.Infof("PFCP Session Deletion Success[%d]\n", SEID)
	} else {
		if smContext.SMContextState == smpcf_context.PFCPModification {
			smContext.SBIPFCPCommunicationChan <- smpcf_context.SessionReleaseFailed
		}
...
func HandlePfcpSessionReportRequest(msg *pfcpUdp.Message) {
	...
	if req.ReportType.Dldr {
		...
		if DLPDR.PDRID == pdrID {
			...
			if n2SmBuf, err := smpcf_context.BuildPDUSessionResourceSetupRequestTransfer(smContext); err != nil {
